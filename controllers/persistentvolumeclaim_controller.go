...
func (r *PersistentVolumeClaimReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	resreq := reconcileContext.instance.Spec.Resources.Requests[corev1.ResourceStorage]
	ressts := reconcileContext.instance.Status.Capacity[corev1.ResourceStorage]

	if resreq.Cmp(ressts) == 1 {
...
func (r *PersistentVolumeClaimReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	// go r.pollMetrics(events)

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1.PersistentVolumeClaim{}).
		WithEventFilter(isAnnotatedPVC).
		//Watches(&source.Channel{Source: events}, isAnnotatedPVC, builder.WatchesOption{}).
		Complete(r)
}

func (r *PersistentVolumeClaimReconciler) getOauthToken(reconcileContext *reconcileContext) string {
...
func (r *PersistentVolumeClaimReconciler) manageError(err error, reconcileContext *reconcileContext) (ctrl.Result, error) {
	r.Recorder.Event(reconcileContext.instance, "Warning", "UnableToExtend", err.Error())
	return reconcile.Result{}, err
}

func (r *PersistentVolumeClaimReconciler) manageSuccess(reconcileContext *reconcileContext) (ctrl.Result, error) {
